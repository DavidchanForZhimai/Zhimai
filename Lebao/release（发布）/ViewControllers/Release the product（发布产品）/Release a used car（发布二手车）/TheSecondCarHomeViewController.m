//
//  TheSecondCarHomeViewController.m
//  Lebao
//
...
- (void)viewDidLoad {
    ...
    _comboBoxDatasource = [[NSArray alloc] initWithObjects:@"整车",@"正前",@"正测",@"车门",@"前排",@"后排",@"中控",@"发动机舱",@"车尾", @"内饰", @"瑕疵",nil];
...
//            NSLog(@"Readdetail =%@ parameter=%@ dataObj =%@",Readdetail,parameter,dataObj);
            if (dataObj) {
                _modal = [TheSecondCarHomeModel mj_objectWithKeyValues:dataObj];
                _modal.datas.acid = _acid;
                if (_modal.rtcode ==1) {
                    
                    [[ToolManager shareInstance] dismiss];
                    for (TheSecondCarCarpics *theSecondEstatepics in _modal.datas.carpics) {
...
- (void)mainView
{
    ...
    __weak TheSecondCarHomeViewController *weakSelf =self;
    ...
    _nextBtn.didClickBtnBlock =^
    {
        ...
        TheSecondCarDetailViewController *next = allocAndInit(TheSecondCarDetailViewController);
...
#pragma mark 二手房源
- (void)reloadImagesView:(TheSecondCarHomeViewController*)weakSelf inView:(UIView*)view
{
