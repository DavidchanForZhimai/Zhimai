//
//  TheSecondCarResultViewController.m
//  Lebao
//
...
//保存产品和房源
#define Saveproduct [NSString stringWithFormat:@"%@product/savecar",HttpURL]

//发布文章
#define CreatearticleUrl  [NSString stringWithFormat:@"%@release/create",HttpURL]
@interface TheSecondCarResultViewController ()<UITextFieldDelegate>
...
@property(nonatomic,strong)UITextView *textFieldone;
...
@implementation TheSecondCarResultViewController
{
...
//                    NSLog(@"_data.amount =%@",_data.amount);
                    [self mainView];
...
//    [UILabel createLabelWithFrame:frame(CGRectGetMaxX(line.frame) + 8 + 4*SpacedFonts*28, frameY(line)+ 2, 6*SpacedFonts*24 ,  24*SpacedFonts) text:@"最多可选3项" fontSize:24*SpacedFonts textColor:LightBlackTitleColor textAlignment:NSTextAlignmentLeft inView:_mainScrollView];
    ...
    NSArray *titles = [NSArray arrayWithObjects:@"无事故",@"有小剐蹭",@"可议价",@"一口价",@"个人一手",@"上下班用车",@"必须过户",@"可随时看车",@"全新车",@"9成新车",@"免过户费",@"可试驾",nil];
    
    for (int i=0;i<titles.count;i++) {
        [_selecteds addObject:@"0"];
    }
//    NSLog(@"_modal.datas.labels =%@",_modal.datas.labels);
    ...
    float titleH = 27;
    float titleXW = 10;
    __weak TheSecondCarResultViewController *weakSelf = self;
    for (int i = 0; i<titles.count; i++) {
...
//                }
    ...
    [UILabel createLabelWithFrame:frame(CGRectGetMaxX(line1.frame) + 4, frameY(line1), 4*SpacedFonts*28,  28*SpacedFonts) text:@"补充说明" fontSize:28*SpacedFonts textColor:BlackTitleColor textAlignment:NSTextAlignmentLeft inView:_mainScrollView];
   
    
    UIView *_projectView = allocAndInitWithFrame(UIView, frame(0, CGRectGetMaxY(line1.frame) + 3, APPWIDTH, 100));
    _projectView.backgroundColor = WhiteColor;
    
    _textFieldone = [[UITextView alloc]initWithFrame:frame(10, 10, APPWIDTH - 20, frameHeight(_projectView) - 20) ];
    [_textFieldone setBorder:LineBg width:0.5];
    [_textFieldone setRadius:5.0];
    [_projectView addSubview:_textFieldone];
//    NSLog(@"_modal.datas.des =%@",_modal.datas.des);
    if (_modal.datas.des) {
        _textFieldone.text = _modal.datas.des;
    }
    //车源标题
    ...
    [UILabel createLabelWithFrame:frame(CGRectGetMaxX(line2.frame) + 4, frameY(line2), 4*SpacedFonts*28,  28*SpacedFonts) text:@"车源标题" fontSize:28*SpacedFonts textColor:BlackTitleColor textAlignment:NSTextAlignmentLeft inView:_mainScrollView];
    ...
    _titleTextField = [self createframe:frame(10, 10, APPWIDTH - 20, (frameHeight(_titletView) - 20)) title:@"请输入车源标题" inView:_titletView];
    _titleTextField.text = _data.pinpaiTitle;
    ...
    UIView *_floorView = allocAndInitWithFrame(UIView, frame(0, CGRectGetMaxY(line3.frame) + 3, APPWIDTH, 80));
    _floorView.backgroundColor = WhiteColor;
    
    weakSelf.feimianBtn = [[BaseButton alloc]initWithFrame:frame(10, 10, 60, 60) backgroundImage:[UIImage imageNamed:@"addition"] iconImage:nil highlightImage:nil inView:_floorView];
    if ([_fengmianImageUrl objectForKey:ImageDiagram]) {
        [[ToolManager shareInstance] imageView:weakSelf.feimianBtn setImageWithURL:[_fengmianImageUrl objectForKey:ImageDiagram] placeholderType:PlaceholderTypeOther];
    }
    ...
    _nextBtn.didClickBtnBlock =^
    {
        ...
        [parameter setObject:@"car" forKey:@"actype"];
        ...
        [parameter setObject:weakSelf.data.model_id forKey:@"model_id"];
        [parameter setObject:weakSelf.data.series_id forKey:@"series_id"];
        [parameter setObject:weakSelf.data.keyid forKey:@"keyid"];
        [parameter setObject:weakSelf.data.color  forKey:@"color"];
        [parameter setObject:weakSelf.data.cartime forKey:@"cartime"];
        [parameter setObject:weakSelf.data.carvenue  forKey:@"carvenue"];
        [parameter setObject:weakSelf.data.mileage forKey:@"mileage"];
        [parameter setObject:weakSelf.data.price  forKey:@"price"];
        [parameter setObject:weakSelf.data.buyprice forKey:@"buyprice"];
        [parameter setObject:weakSelf.textFieldone.text  forKey:@"description"];
...
@implementation TheSecondCarResult
