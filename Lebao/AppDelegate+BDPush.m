...
- (void)notifacionApi:(NSDictionary *)notifacion isAleat:(BOOL)isAleat
{
    ...
    for ( UINavigationController * subnav in self.mainTab.viewControllers) {
        for (UIViewController *subVC in subnav.viewControllers) {
            ...
            if ([subVC isKindOfClass:[WBHomePageVC class]]) {
                
                WBHomePageVC *wBHomePageVC = (WBHomePageVC *)subVC;
                [wBHomePageVC.homePageBtn setImage:[UIImage imageNamed:@"icon_dicover_me_selected"] forState:UIControlStateNormal];
    ...
    {
        //如果是当前控制器是我的消息控制器的话，刷新数据即可
        if([baseVC isKindOfClass:[WBHomePageVC class]])
        {
