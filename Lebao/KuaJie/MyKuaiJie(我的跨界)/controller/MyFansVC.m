...
-(void)getJson
{
    [[MyKuaJieInfo shareInstance]getFansLieBiaoWithPage:_page andCallBack:^(BOOL issucced, NSString *info, NSArray *jsonArr) {
        if (issucced == YES) {
            if (jsonArr.count >0) {
                ...
                _titLab.text = [NSString stringWithFormat:@"粉丝(%ld)",_jsonArr.count];
                [_myTab reloadData];
            }else
            {
